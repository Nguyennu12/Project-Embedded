...
void runDirection(void)
{
    ...
    if (value != oldDirection)
    {
        ...
        switch(value)
        {
            case 0b00011111: // Cả hai cảm biến đều sáng
                RmotorDCStop(); /* Dừng cả 2 động cơ */
                LmotorDCStop();
                delayms(50);

                // Nếu cảm biến bên trái sáng
                if ((value & 0b00000001) == 0b00000001)
                {
                    LmotorDCGoBackward(); // Lùi động cơ trái
                    RmotorDCGoForward();  // Tiến động cơ phải
                ...
                // Nếu cảm biến bên phải sáng
                else if ((value & 0b00000010) == 0b00000010)
                {
                    LmotorDCGoForward();  // Tiến động cơ trái
                    RmotorDCGoBackward(); // Lùi động cơ phải
                    FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM);
                    FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM);
                    FTM_SetSoftwareTrigger(FTM0, true);
                }
            ...
            case 0b00011110: /* Lệch phải 1 */
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM - DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM);
                break;
            case 0b00011100: /* Lệch phải 2 */
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM - DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM);
                break;
            case 0b00011001: /* Lệch phải 3 */
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM - DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM);
                break;

            case 0b00011011: /* Không lệch */
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM);
                break;

            case 0b00010011: /* Lệch trái 1 */
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM - DCPWM);
                break;
            case 0b00000111: /* Lệch trái 2 */
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM - DCPWM);
                break;
            case 0b00001111: /* Lệch trái 3 */
                FTM_UpdatePwmDutycycle(FTM0, LEFTDC, kFTM_CenterAlignedPwm, DCPWM);
                FTM_UpdatePwmDutycycle(FTM0, RIGHTDC, kFTM_CenterAlignedPwm, DCPWM - DCPWM);
                break;
        }
